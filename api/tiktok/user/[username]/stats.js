export default async function handler(req, res) {
  ...
  try {
    ...
    const stats = JSON.parse(match[1])['__DEFAULT_SCOPE__']['webapp.user-detail'].userInfo.stats;
    res.status(200).json({ success: true, username, stats, timestamp: new Date().toISOString() });
  } catch (err) {
